...
| <a name="input_cloudwatch-logs"></a> [cloudwatch-logs](#input\_cloudwatch-logs) | List of Cloudwatch Logs to add a Subscription Filters for to ESF | <pre>list(object({<br>    arn = string<br>  }))</pre> | <pre>[<br>  {<br>    "arn": ""<br>  }<br>]</pre> | no |
| <a name="input_config-file"></a> [config-file](#input\_config-file) | ARN of to the location of config.yaml for ESF in S3 | `string` | n/a | yes |
